...
def extended_trace(func=None, redact=[]):
    ...
    '''
    This small decorator will trace the execution of your code every time it enters or exits 
    a decorated function (by thread) and will insert appropriate indent into the log file 
    along with exception information.
    '''
    import threading
    import traceback
...
def trace(func=None, log_return=False):
    ...
    '''
    This is a decorator that logs function arguments, return object and time taken for execution as well as any 
    exception that might have been raised by the function. Useful for debug logging.
    '''
    ...
    def wrapper(*args, **kw):
        ...
        if log_return:
            logger.debug(
                "name=%s args=%s, kw=%s, time=%s" % (func.__name__, args, kw, ret, time_taken)
            )
        else:
            logger.debug(
                "name=%s, args=%s, kw=%s, time=%s" % (func.__name__, args, kw, time_taken)
            )
        if e:
            logger.exception(e)
            raise e
